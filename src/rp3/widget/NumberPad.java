...
public class NumberPad extends FrameLayout {
	...
	public NumberPad(Context context, AttributeSet attrs) {
		...
		if (!isInEditMode()) {			
			...
			TypedArray a = context.getTheme().obtainStyledAttributes(attrs,
					R.styleable.NumberPad, 0, 0);

			try {
				targetViewId = a.getResourceId(
						R.styleable.NumberPad_targetView, 0);
	...
	public void setTargetView(int resId) {
		...
		if (targetViewId != 0)
			// targetView = (TextView)
			// ((Activity)this.context).getWindow().getDecorView().findViewById(targetViewId);
