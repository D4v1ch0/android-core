...
public class SlidingPaneLayout extends ViewGroup {
	...
	protected boolean canScroll(View v, boolean checkV, int dx, int x, int y) {
		...
		/*
		 * special case ViewPagers, which don't properly implement the scrolling
		 * interface
		 */
		return checkV
				&& (ViewCompat.canScrollHorizontally(v, -dx) || ((v instanceof ViewPager) && canViewPagerScrollHorizontally(
						(ViewPager) v, -dx)));
	}

	boolean canViewPagerScrollHorizontally(ViewPager p, int dx) {
		return !(dx < 0 && p.getCurrentItem() <= 0 || 0 < dx
				&& p.getAdapter().getCount() - 1 <= p.getCurrentItem());
	}
}
